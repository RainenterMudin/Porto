import Link from "next/link";
import type { Metadata } from "next";
import { ArrowLeft, Ticket } from "lucide-react";
import { Navbar } from "@/components/Navbar";
import { Footer } from "@/components/Footer";
import { TicketingApp } from "@/components/ticketing/TicketingApp";
import { profile } from "@/lib/data";

export const metadata: Metadata = {
  title: `Ticketing Travel — ${profile.name}`,
  description:
    "Sistem pemesanan tiket travel/shuttle: cari jadwal, pilih kursi lewat seat map, isi data penumpang, dan dapatkan e-tiket dengan kode booking.",
};

export default function TicketingPage() {
  return (
    <>
      <Navbar />
      <main className="mx-auto w-full max-w-4xl px-6 pb-20 pt-28">
        <Link
          href="/"
          className="mb-8 inline-flex items-center gap-2 text-sm font-medium text-zinc-500 transition hover:text-brand-500 dark:text-zinc-400"
        >
          <ArrowLeft className="h-4 w-4" />
          Kembali ke beranda
        </Link>

        <div className="mb-8 flex items-start gap-4">
          <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-500 to-fuchsia-500 text-white">
            <Ticket className="h-6 w-6" />
          </div>
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-zinc-900 sm:text-4xl dark:text-white">
              Ticketing Travel
            </h1>
            <p className="mt-2 max-w-2xl text-zinc-500 dark:text-zinc-400">
              Alur pemesanan tiket travel/shuttle yang lengkap: cari jadwal
              antar kota, pilih kursi lewat seat map interaktif, isi data
              penumpang, lalu terbitkan e-tiket dengan kode booking.
            </p>
          </div>
        </div>

        <TicketingApp />
      </main>
      <Footer />
    </>
  );
}
